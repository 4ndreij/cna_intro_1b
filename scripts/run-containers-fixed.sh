...
# This script runs the microservices in containers with Dapr sidecars
...
podman run -d \
    ...
    -e DAPR_GRPC_ENDPOINT=productservice-dapr:50001 \
...
podman run -d \
    ...
    -e DAPR_GRPC_ENDPOINT=orderservice-dapr:50001 \
    -v "$(pwd)/infrastructure/dapr:/dapr/components:ro" \
    $ORDER_IMAGE
